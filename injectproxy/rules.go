	...
	for _, rg := range rgs.RuleGroups {
		...
		for _, rgr := range rg.Rules {
			...
			for i := range rgr.alertingRule.Alerts {
				...
				if ar == nil {
					// TODO(simonpasquier): decide if we should modify the
					// expression or delegate to the enforcement to the query
					// endpoint.
					...
					//TODO(simonpasquier): remove it.
					ar.Query = rgr.alertingRule.Query
				}
